...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256, npy_out_fnames=['CBCT2CT']):
    ...
    for label, im_data in visuals.items():
        ...
        if 'fake_B' in label: # CBCT to CT translated image
            image_numpy = (im_data[0].cpu().float().numpy())[0]  # convert it into a numpy array
            suffix = npy_out_fnames[idx] + '.nrrd'
            if 'RTSTRUCTS' in suffix:
                # RT structs should be rescaled 0 to 5 range
                image_numpy = ((image_numpy + 1.) / 2.) * 4.
                image_numpy = np.round(image_numpy)
                image_sitk = sitk.GetImageFromArray(image_numpy.astype(np.uint8))
            else:
